        ...
        <div class="card-body">
          <div class="media">
            ...
            <div class="media-body">
              ...
              <div>${i18n.getMessage("theme.default.user.profile.1")}：<a href="/top100">${user.score}</a></div>
                <#if user.bio??>
                  <div><i class="gray">${user.bio!}</i></div>
                </#if>
              <div>${i18n.getMessage("theme.default.user.profile.2")}: <a href="/user/${user.username}/collects">${collectCount!0}</a></div>
              <div>${i18n.getMessage("theme.default.user.profile.3")}: ${model.formatDate(user.inTime)}</div>
            </div>
          </div>
        </div>
